...
for file in $INDIR/*${TYPE}_1.fq.gz; do
    ...
    if [ ! -f "$OUTNAME/abundance.h5" ];  then
            ...
            COMMAND1="$KALLISTO quant -i $INDEX_FILE -o ${OUTNAME} -b 100 --pseudobam --threads=8 ${file} ${FILENAME}_2.fq.gz  > pseudoal.sam 2>>$LOGFILE"
            COMMAND2="$KALLISTO h5dump -o ${OUTNAME}  ${OUTNAME}/abundance.h5"
            TIME=$( `time -p sh -c "$COMMAND1; $COMMAND2"` 2>&1 )
            echo $COMMAND1
            echo $TIME
            mv pseudoal.sam $OUTNAME/pseudoal.sam
